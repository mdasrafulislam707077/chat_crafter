...
export async function intentPost(req: Request, res: Response) {
  const data = req.body;
  const file = await fileExists(`${fileBasepath.intents(req.body?.botname)}${data?.name}.json`);
  if (file) {
    return res.status(401).json({ status: 'failed' });
  } else {
    const datas = { info: data, items: [], id: uuid(), timeSP: Date.now() };
    fs.writeFile(`src/doc/${req.body?.botname}/intents/${data?.name}.json`, JSON.stringify(datas));
